import React from "react";
import PropTypes from "prop-types";
import NavbarScreen from "../../components/NavBar/Navbar-screen";
import Footer_v2 from "../../components/Footerv2/Footer_v2";
import { BellOutlined, UserOutlined } from "@ant-design/icons";
import { Avatar, Badge, Tabs, Table } from "antd";

const { TabPane } = Tabs;
NoteScreen.propTypes = {};

function NoteScreen(props) {
  const columns = [
    {
      title: "Vaccine",
      dataIndex: "Vaccine",
      key: "Vaccine",
      align: "center",
      render: (value) => (
        <span className="schedule__vaccine-name"> {value}</span>
      ),
    },
    {
      title: "1",
      dataIndex: "time1",
      key: "time1",
      align: "center",
      render: (value) => {
        if (value)
          return (
            <div
              className={
                value.daTiem
                  ? "schedule__vaccine-isInjection"
                  : "schedule__vaccine-noInjection"
              }
            >
              <span className="schedule__vaccine-date"> {value.date}</span>
              <br />
              <span className="  schedule__vaccine-injections">
                {value.daTiem ? "Đã tiêm" : "Chưa tiêm"}
              </span>
            </div>
          );
        else {
          return <></>;
        }
      },
    },
    {
      title: "2",
      dataIndex: "time2",
      key: "time2",
      align: "center",
      render: (value) => {
        if (value)
          return (
            <div
              className={
                value.daTiem
                  ? "schedule__vaccine-isInjection"
                  : "schedule__vaccine-noInjection"
              }
            >
              <span className="schedule__vaccine-date"> {value.date}</span>
              <br />
              <span className="  schedule__vaccine-injections">
                {value.daTiem ? "Đã tiêm" : "Chưa tiêm"}
              </span>
            </div>
          );
        else {
          return <></>;
        }
      },
    },
    {
      title: "3",
      dataIndex: "time3",
      key: "time3",
      align: "center",
      render: (value) => {
        if (value)
          return (
            <div
              className={
                value.daTiem
                  ? "schedule__vaccine-isInjection"
                  : "schedule__vaccine-noInjection"
              }
            >
              <span className="schedule__vaccine-date"> {value.date}</span>
              <br />
              <span className="  schedule__vaccine-injections">
                {value.daTiem ? "Đã tiêm" : "Chưa tiêm"}
              </span>
            </div>
          );
        else {
          return <></>;
        }
      },
    },
    {
      title: "4",
      dataIndex: "time4",
      key: "time4",
      align: "center",
      render: (value) => {
        if (value)
          return (
            <div
              className={
                value.daTiem
                  ? "schedule__vaccine-isInjection"
                  : "schedule__vaccine-noInjection"
              }
            >
              <span className="schedule__vaccine-date"> {value.date}</span>
              <br />
              <span className="  schedule__vaccine-injections">
                {value.daTiem ? "Đã tiêm" : "Chưa tiêm"}
              </span>
            </div>
          );
        else {
          return <></>;
        }
      },
    },
    {
      title: "5",
      dataIndex: "time5",
      key: "time5",
      align: "center",
      render: (value) => {
        if (value)
          return (
            <div
              className={
                value.daTiem
                  ? "schedule__vaccine-isInjection"
                  : "schedule__vaccine-noInjection"
              }
            >
              <span className="schedule__vaccine-date"> {value.date}</span>
              <br />
              <span className="  schedule__vaccine-injections">
                {value.daTiem ? "Đã tiêm" : "Chưa tiêm"}
              </span>
            </div>
          );
        else {
          return <></>;
        }
      },
    },
  ];
  const data = [
    {
      key: "1",
      Vaccine: "JEEV",
      time1: { date: "11/07/2020", daTiem: true },
      time2: { date: "08/09/2020", daTiem: true },
      time3: { date: "08/10/2020", daTiem: false },
      time4: "",
      time5: "",
    },

    {
      key: "2",
      Vaccine: "GC FLU",
      time1: { date: "11/07/2020", daTiem: true },
      time2: { date: "08/09/2020", daTiem: true },
      time3: { date: "08/10/2020", daTiem: false },
      time4: { date: "08/10/2020", daTiem: false },
      time5: { date: "08/10/2020", daTiem: false },
    },
    {
      key: "3",
      Vaccine: "Engerix B",
      time1: { date: "11/07/2020", daTiem: true },
      time2: { date: "08/09/2020", daTiem: true },
      time3: { date: "08/10/2020", daTiem: false },
      time4: { date: "08/10/2020", daTiem: false },
      time5: "",
    },
  ];

  return (
    <>
      <NavbarScreen />
      <div className="note__screen">
        <div className="note__header container-fluid">
          <div className="header__title">
            <img
              src={
                process.env.PUBLIC_URL +
                "/images/Icon/vaccination_book_icon.png"
              }
              alt=""
            />
            <div>Sổ tiêm chủng</div>
          </div>
          <div className="header__name">
            <img
              src={process.env.PUBLIC_URL + "/images/Icon/family_icon.png"}
              alt=""
            />
            <div>Nguyễn Văn Hiếu</div>
          </div>
          <div className="header__action">
            <div className="action__bell">
              <Badge count={5} size="large">
                <a href="#" className="head-example" />
                <BellOutlined />
              </Badge>
            </div>
            <div className="action__language">
              <img
                src={process.env.PUBLIC_URL + "/images/Icon/language_icon.png"}
                alt=""
              />
            </div>
            <div className="action__avatar">
              <Avatar icon={<UserOutlined />} />
              <div className="action__avatar--number">0903420066</div>
            </div>
          </div>
        </div>

        <div className="note__content container-fluid">
          <div className="content__name">
            <div className="name__item">
              <div className=" name__item-avatar">
                <img src="data:image/jpeg;base64,"></img>
              </div>
              <div className="name__item-name selected">Nguyễn Văn Hiếu</div>
            </div>

            <div className="name__item">
              <div className=" name__item-avatar">
                <img src="data:image/jpeg;base64,"></img>
              </div>
              <div className="name__item-name">Nguyễn Khánh Lâm</div>
            </div>
          </div>
          <div className="content__info">
            <div className="info__common row p-0 m-0">
              <div className="info__common-avatar col-4">
                <img src="data:image/jpeg;base64,"></img>
              </div>
              <div className=" col-8 ml-0 pl-0  ">
                <div className="row">
                  <div className="info__common-name col-12">
                    Nguyen Van Hieu
                  </div>

                  <div className="info__common-qr col-4">
                    <img
                      src="https://uploads-ssl.webflow.com/577f4c48e970a6062647c960/606569452d86c0feb450cdea_m%C3%A3%20v%E1%BA%A1ch.png"
                      alt=""
                    />
                  </div>
                  <div className="info__common-codeQr col-8">SFP001193847</div>
                </div>
              </div>
            </div>

            <div className="info__detail row p-0 m-0">
              <div className="info__detail-title col-12 p-3">
                Thông tin chung
              </div>

              <div className="info__detail-name col-5 pl-3">Họ và tên</div>
              <div className="info__detail-name col-7 p-0">Nguyễn Văn Hiếu</div>

              <div className="info__detail-name col-5 pl-3">Ngày sinh</div>
              <div className="info__detail-name col-7 p-0">30/01/1970</div>

              <div className="info__detail-name col-5 pl-3">Giới tính</div>
              <div className="info__detail-name col-7 p-0">Nam</div>

              <div className="info__detail-name col-5 pl-3">Điện thoại</div>
              <div className="info__detail-name col-7 p-0">0903420066</div>

              <div className="info__detail-name col-5 pl-3">Email</div>
              <div className="info__detail-name col-7 p-0">
                Hieunguyen@hanoisoftware.net
              </div>

              <div className="info__detail-name col-5 pl-3">Quan hệ</div>
              <div className="info__detail-name col-7 p-0">Bố</div>

              <div className="info__detail-title col-12 p-3">
                Địa chỉ đăng ký
              </div>

              <div className="info__detail-name col-5 pl-3">Địa chỉ</div>
              <div className="info__detail-name col-7 p-0">
                68 Hoa Sữa 5, Vinhome Riveside
              </div>

              <div className="info__detail-name col-5 pl-3">Thôn/ ấp</div>
              <div className="info__detail-name col-7 p-0"></div>

              <div className="info__detail-name col-5 pl-3">Xã/Phường</div>
              <div className="info__detail-name col-7 p-0">Phúc Lợi</div>

              <div className="info__detail-name col-5 pl-3">Quận/Huyện</div>
              <div className="info__detail-name col-7 p-0">Long Biên</div>

              <div className="info__detail-name col-5 pl-3">Tỉnh/ Thành</div>
              <div className="info__detail-name col-7 p-0">Hà Nội</div>

              <div className="info__detail-name col-5 pl-3">Quốc Gia</div>
              <div className="info__detail-name col-7 p-0">Việt Nam</div>
            </div>
          </div>
          <div className="content__schedule px-2">
            <Tabs defaultActiveKey="1">
              <TabPane tab="Vaccine tiêm chủng" key="1">
                <Table
                  pagination={false}
                  size="small"
                  bordered
                  columns={columns}
                  dataSource={data}
                  scroll={{ x: 770 }}
                />
              </TabPane>
              <TabPane tab="Lịch tiêm đã đăng kí" key="2">
                Tab 2
              </TabPane>
            </Tabs>
          </div>
        </div>
      </div>

      <Footer_v2 />
    </>
  );
}

export default NoteScreen;
